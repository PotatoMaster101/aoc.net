...
public class PositionTest
{
    ...
    public void GetSquaredEuclideanDistance_ReturnsCorrectValue(int x1, int y1, int x2, int y2, int expected)
    {
        // arrange
        var sut = new Position<int>(x1, y1);

        // act
        var result = sut.GetSquaredEuclideanDistance(new Position<int>(x2, y2));
    ...
    [InlineData(0, 0)]
    [InlineData(-3, 9)]
    public void ToVectorConversion_ReturnsCorrectValue(int x, int y)
    {
        // arrange
        var sut = new Position<int>(x, y);
    ...
    [InlineData(0, 0)]
    [InlineData(-3, 9)]
    public void FromVectorConversion_ReturnsCorrectValue(float x, float y)
    {
        // arrange
        var sut = new Vector2(x, y);

        // act
        var result = (Position<float>)sut;
    ...
    private class GetNeighboursTestData : IEnumerable<object[]>
    {
        ...
        public IEnumerator<object[]> GetEnumerator()
        {
            ...
            yield return [0, 0, 1, Array.Empty<Direction>(), Array.Empty<Position<int>>()];
            yield return [0, 0, 1, null!, new Position<int>[] { new(0, -1), new(0, 1), new(-1, 0), new(1, 0) }];
            yield return [0, 0, 0, null!, new Position<int>[] { new(0, -1), new(0, 1), new(-1, 0), new(1, 0) }];
            yield return [0, 0, -1, null!, new Position<int>[] { new(0, -1), new(0, 1), new(-1, 0), new(1, 0) }];
        }
    }
}
