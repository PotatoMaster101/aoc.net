    ...
    where T : struct
{
    ...
    public GridEntry<T> this[int i] => new(i.ToPosition(Width), _data[i]);
    ...
    public IEnumerator<GridEntry<T>> GetEnumerator()
    {
        ...
        for (var i = 0; i < _data.Count; i++)
            yield return new GridEntry<T>(i.ToPosition(Width), _data[i]);
    }
    ...
    public GridEntry<T>? Find(T value)
    {
        ...
        return i < 0 ? null : new GridEntry<T>(i.ToPosition(Width), value);
    }
    ...
    public IEnumerable<GridEntry<T>> FindAll(T value)
    {
        for (var i = 0; i < _data.Count; i++)
            if (_data[i].Equals(value))
                yield return new GridEntry<T>(i.ToPosition(Width), value);
    }
