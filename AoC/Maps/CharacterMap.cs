...
/// </summary>
[DebuggerDisplay("{Grid.Area.Width} x {Grid.Area.Height}")]
...
public class CharacterMap : Map<char>
{
    ...
    public virtual bool IsWall(Position<int> position)
    {
        return Grid[position].Value == '#';
    }

    /// <summary>
    /// Determines whether a position is a path tile.
    /// </summary>
    /// <param name="position">The position to check.</param>
    /// <returns>Whether <paramref name="position"/> is a path tile.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public virtual bool IsPath(Position<int> position)
    {
        return Grid[position].Value != '#';
    }
}
